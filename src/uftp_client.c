...
#define USAGE_ERROR(S) {fprintf(stderr, "Usage: %s [IP ADDR] [PORT #]\n", S); exit(0);} //usage error message
...
int main(int argc, char** argv) {
    ...
    if ( *ptr != '\0') { fprintf(stderr, "Please enter a valid port number]n"); exit(0); }
    ...
    if (server == NULL) { fprintf(stderr, "Invalid address: %s\n", argv[1]); exit(0);}
